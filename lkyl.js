  ...
  return new Promise((resolve, reject) => {
    if (taskstatus.data.dailyTasks[1].status != "received") {
      ...
      for (j = 0; j < 4; j++) {
        ...
        $.post(videourl, function (error, response, data) {
          if (logs) $.log(`${cookieName}, 视频: ${data}`);
        });
        $.get(videotaskurl, function (error, response, data) {
          if (logs) $.log(`${cookieName}, 视频银豆: ${data}`);
        });
      }
    }
    if (taskstatus.data.dailyTasks[1].status == "received") {
  ...
  return new Promise((resolve, reject) => {
    ...
    $.get(daytaskurl, (error, response, data) => {
      ...
      if (Incomplete > 0) {
        for (
          k = 0;
          k < task.data.homeActivities.length &&
          task.data.homeActivities[k].participated == false;
          k++
        ) {
      ...
      $.get(hinturl, (error, response, data) => {
        ...
        if (SilverBean > excresult.datas[0].salePrice) {
          for (k = 0; k < excresult.datas.length; k++) {
            ...
            if (
              SilverBean < excresult.datas[k].salePrice &&
              SilverBean > excresult.datas[k - 1].salePrice
            ) {
              detail +=
                Silvertotal +
                `${excresult.datas[k - 1].salePrice}银豆兑换${
                  excresult.datas[k - 1].productPrice
                }京豆\n`;
            } else if (excresult.datas[k].salePrice == SilverBean) {
              detail +=
                Silvertotal +
                `${excresult.datas[k].salePrice}银豆兑换${excresult.datas[k].productPrice}京豆\n`;
            }
          }
          ...
          detail +=
            Silvertotal +
            `${excresult.datas[0].salePrice}银豆随机兑换${excresult.datas[0].productName}\n`;
        }
        resolve();
      });
    });
